...
export function registerDesignSpecTools(server) {
    server.tool("get_design_spec_prompt", "Returns a ready-to-use prompt for generating a comprehensive UI/UX design specification. The spec covers semantic color tokens (light + dark), typography scale, spacing system, component dimensions, motion guidelines, state patterns, and accessibility requirements — everything needed to guide implementation of the presentation layer.", {
        ...
        moodBoardAnalysis: z
            .string()
            .default("")
            .describe("Visual analysis from the mood board — extracted palette, visual metaphors, atmosphere. Leave empty if skipped."),
...
${DESIGN_SPEC_SCHEMA}`;
    ...
    server.tool("save_design_spec", "Save a UI/UX design specification JSON to disk so it can be loaded by get_final_code_prompt as implementation guidance.", {
        designSpecJson: z
            .string()
        ...
        outputPath: z
            .string()
            .optional()
            .describe("File path to save to (default: OUTPUT_DIR/design-spec.json)"),
    }, async ({ designSpecJson, outputPath }) => {
        const filePath = outputPath ?? path.join(env.outputDir, "design-spec.json");
    ...
    server.tool("load_design_spec", "Load a previously saved design spec from disk. Use the result as the designSpec input to get_final_code_prompt.", {
        inputPath: z
            .string()
            .optional()
            .describe("File path to read from (default: OUTPUT_DIR/design-spec.json)"),
    }, async ({ inputPath }) => {
        const filePath = inputPath ?? path.join(env.outputDir, "design-spec.json");
        try {
