...
function resolveReviewPath(reviewPath) {
    if (path.isAbsolute(reviewPath)) {
        return reviewPath;
    }
    return path.join(env.outputDir, reviewPath);
}
function buildReviewPrompt(reviewFocus, brandGuide, designSpec, screenshotLabel) {
...
async function loadScreenshot(resolvedPath) {
    await fs.access(resolvedPath);
    return fs.readFile(resolvedPath);
}
export function registerUiReviewTools(server) {
    server.tool("review_ui_screenshot", "Read a rendered UI screenshot and return it directly to the calling multimodal AI with a structured review prompt. Use this after generating and rendering code to critique visual quality, brand alignment, hierarchy, density, and anti-pattern drift.", {
        screenshotPath: z
            .string()
            .describe("Screenshot path. Absolute paths are allowed; relative paths are resolved from OUTPUT_DIR."),
        brandGuide: z
            .string()
            .default("")
        ...
        const cleanDesignSpec = designSpec.trim()
            ...
            : "Not provided.";
        const reviewPrompt = buildReviewPrompt(reviewFocus, cleanBrandGuide, cleanDesignSpec, path.basename(resolvedPath));
        return {
            content: [
    ...
    server.tool("review_ui_screenshot_dir", "Read all screenshots from a directory and return them to the calling multimodal AI with a structured review prompt for flow-level comparison. Use this for multi-screen review after rendering a prototype or app flow.", {
        screenshotDir: z
            .string()
            .describe("Directory containing screenshots. Absolute paths are allowed; relative paths are resolved from OUTPUT_DIR."),
        brandGuide: z
            .string()
            .default("")
            .describe("Optional brand guide JSON string to review against."),
...
- Mention if the flow drifts between multiple UI styles.`;
    ...
    server.tool("save_ui_review", "Save a structured UI review JSON result to disk so it can be reused in later design or code refinement steps.", {
    ...
    server.tool("load_ui_review", "Load a previously saved UI review JSON from disk.", {
        inputPath: z
            .string()
            .optional()
...
//# sourceMappingURL=ui-review.js.map