...
export function registerBrandGuideTools(server) {
    server.tool("get_brand_guide_prompt", "Returns the brand guide JSON schema and a ready-to-use prompt. Call this first, then use the prompt yourself to generate the brand guide — no internal AI call needed.", {
        ...
        coreEmotion: z
            .string()
            .describe("Primary emotion(s) users should feel — one phrase or several comma-separated keywords"),
        visualMetaphorsHint: z
            .string()
            .optional()
            .describe("Optional: real-world objects, materials, or spaces that could inspire the UI (e.g. 'aged leather notebook, warm candlelight'). Leave empty to let the AI derive them."),
        antiGoalsHint: z
            .string()
            .optional()
            .describe("Optional: the look/feel this app must NEVER have (e.g. 'cold corporate SaaS dashboard, hyper-saturated social media feed'). Leave empty to let the AI derive them."),
...
${BRAND_GUIDE_SCHEMA}`;
    ...
    server.tool("save_brand_guide", "Save a brand guide JSON to a local file so it can be referenced by later steps (palette extraction, code generation, etc.).", {
        brandGuideJson: z.string().describe("The brand guide as a JSON string"),
        outputPath: z
            .string()
            .optional()
            .describe("File path to save to (default: OUTPUT_DIR/brand-guide.json)"),
    }, async ({ brandGuideJson, outputPath }) => {
        const targetPath = outputPath ?? path.join(env.outputDir, "brand-guide.json");
        ...
        return {
            content: [
                {
                    type: "text",
                    text: JSON.stringify({ saved: true, path: path.resolve(targetPath) }, null, 2),
                },
            ],
        };
    });
    server.tool("load_brand_guide", "Load a previously saved brand guide JSON from disk.", {
        filePath: z
            .string()
            .optional()
            .describe("Path to brand guide JSON (default: OUTPUT_DIR/brand-guide.json)"),
    }, async ({ filePath }) => {
        const targetPath = filePath ?? path.join(env.outputDir, "brand-guide.json");
        try {
